...
import { LayoutDashboard, User, Settings, LogOut, Plus, ExternalLink, Trash2, Menu, X } from "lucide-react";
...
export default function DashboardClient({ initialUser }: { initialUser: any }) {
  ...
  const navItems = [
    ...
    { href: "/dashboard/profile", icon: User, label: "Perfil", active: false },
        ...
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <nav className="absolute left-0 top-0 bottom-0 w-72 bg-white dark:bg-[#0a0a0a] border-r border-[#eeeeee] dark:border-[#222] p-6 flex flex-col shadow-2xl">
            <div className="flex items-center justify-between mb-10">
              ...
              <button
                onClick={() => setIsMobileNavOpen(false)}
                className="p-2 hover:bg-gray-100 dark:hover:bg-[#111] rounded-full transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
      ...
      <main className="flex-1 p-5 md:p-12 min-w-0">
        ...
        <div className="flex items-center gap-4 mb-8 md:hidden">
          <button
            ...
            className="p-2.5 border border-[#eeeeee] dark:border-[#222] rounded-xl hover:bg-gray-50 dark:hover:bg-[#111] transition-colors"
          >
        ...
        <header className="hidden md:flex justify-between items-start mb-12">
          ...
          <Link
            href="/auth/signout"
            className="p-3 border border-[#eeeeee] dark:border-[#222] rounded-full hover:bg-gray-100 dark:hover:bg-[#111] transition-colors"
          >
        ...
        {loading ? (
          ...
          <div className="grid gap-4">
            {links.map((link) => (
              ...
              >
                <div className="w-12 h-12 bg-[#f9fafb] dark:bg-[#111] rounded-xl flex items-center justify-center border border-[#eeeeee] dark:border-[#222] flex-shrink-0">
